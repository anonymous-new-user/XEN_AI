/**
 * Xen AI - Advanced AI Assistant
 * v2.8.0 (Updated: AI Pipe integration + bug fixes)
 * Author: Pawan Sirigiri (Original) & Assistant (fixes)
 */
class XenAI {
    constructor() {
        this.version = '2.8.0';
    ...
    async init() {
        try {
            ...
            console.log(`XenAI v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize XenAI:', error);
...
hideLoadingScreen() {
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'XenAI', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to XenAI! How can I assist you?');
        }
    }
